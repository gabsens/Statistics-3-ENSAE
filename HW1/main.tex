...
\begin{enumerate}
  \item We recall two useful lemmas about convergence:\newline
  \underline{Lemma 3}: $(X_n^1,\ldots,X_n^d)$ converges almost surely to $(X^1,\ldots,X^d)$ if and only if each real r.v. $X_n^i$ converges almost surely to $X^i$.\newline
  \underline{Lemma 4}: $(X_n^1,\ldots,X_n^d)$ converges in probability to $(X^1,\ldots,X^d)$ if and only if each real r.v. $X_n^i$ converges in probability to $X^i$. \newline

  By Lemmas $3$ and $4$ it suffices to prove the claim in the case $d=1$.\newline
  Let $A = \{w\in \Omega,\; T_n(w)\xrightarrow[n\to \infty]{} T(w)\}$. Note that $A = \bigcap_{m\geq 1} \bigcup_{n\geq 1} \bigcap_{k\geq n} |T_k-T|\geq \frac 1m$.\newline By assumption, $P(A^c)=0$, hence $P(\bigcup_{m\geq 1} \bigcap_{n\geq 1} \bigcup_{k\geq n} |T_k-T|\geq \frac 1m) = 0$, which implies $\forall m\geq 1, P(\bigcap_{n\geq 1} \bigcup_{k\geq n} |T_k-T|\geq \frac 1m) = 0$.
  ...
  \item \underline{Lemma 5}: $T_n$ converges to $T$ in probability if and only if $E(\min(\|T_n-T\|_2,1))\xrightarrow[n\to \infty]{} 0$.\newline
  \textit{Proof}: $\Rightarrow$ Let $\varepsilon >0$ and note that $$\begin{aligned}[t]
    E(\min(\|T_n-T\|_2,1)) &= E(\min(\|T_n-T\|_2,1)1_{\|T_n-T\|_2\leq \varepsilon}) + E(\min(\|T_n-T\|_2,1)1_{\|T_n-T\|_2> \varepsilon})\\
    &\leq E(\|T_n-T\|_2 1_{\|T_n-T\|_2\leq \varepsilon}) + E(1_{\|T_n-T\|_2> \varepsilon})\\
    &\leq \varepsilon + P(\|T_n-T\|_2> \varepsilon)
  \end{aligned}$$
  Taking the $\limsup$ on both side yields $\limsup_n E(\min(\|T_n-T\|_2,1))\leq \varepsilon$ and letting $\varepsilon\to 0$ finishes the proof.\newline
  $\Leftarrow$ Note that $E(\min(\|T_n-T\|_2,1)) = E(\|T_n-T\|_2 1_{\|T_n-T\|_2 < 1} + 1_{\|T_n-T\|_2 \geq 1})\geq E(1_{\|T_n-T\|_2 \geq 1})$, hence $P(\|T_n-T\|_2 \geq 1)\xrightarrow[n\to \infty]{} 0$. Let $\varepsilon \in (0,1)$. It's not hard to check that $$P(\|T_n-T\|_2 \geq \varepsilon) = P(\min(\|T_n-T\|_2,1)\geq \varepsilon)$$ and Markov's inequality yields $P(\|T_n-T\|_2 \geq \varepsilon) \leq \frac{E(\min(\|T_n-T\|_2,1))}{\varepsilon} \xrightarrow[n\to \infty]{} 0$. In the case $\varepsilon \geq 1$, simply note that $P(\|T_n-T\|_2 \geq \varepsilon)\leq P(\|T_n-T\|_2 \geq 1)\xrightarrow[n\to \infty]{} 0$.
  \hfill $\square$
  \\ \\
  If $T=t$ a.s, note that the function $x\mapsto \min(\|x-t\|_2,1)$ is continuous and bounded, thus $E(\min(\|T_n-t\|_2,1)) \xrightarrow[n\to \infty]{} E(\min(\|t-t\|_2,1)) = 0$. By Lemma 5, $T_n$ converges in probability to $t$.
\end{enumerate}
